...
public class ACIncreaseAbility : AbilityClassScript
{
    ...
    public override int useAbility(float abilityRange, LayerMask mask, int amount = 0)
    {
        ...
        foreach (GameObject g in effectedList)
        {
            ...
            {
                w.tempACBuff(Mathf.RoundToInt(Random.Range(1, amount)) + Mathf.RoundToInt(amount / 2f));
            }
